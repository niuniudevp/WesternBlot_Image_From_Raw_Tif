from PyQt5.QtWidgets import QFrame, QComboBox, QDialog, QDialogButtonBox, QFormLayout, QPushButton, QVBoxLayout, QTextEdit, QHBoxLayout, QLabel, QFileDialog, QInputDialog, QLineEdit
...
class Bound_Split_Dialog_Preview(Muliti_Input_Dialog):
    ...
    def update_preview(self):
        ...
        for i in range(ll):
            ...
            cv_img = cv2.line(cv_img, (bounds_x_fixed[start] + 10, bounds_y_fixed[0]), (bounds_x_fixed[end] - 10, bounds_y_fixed[1]), (0, 0, 255), 2)
            current = end
...
class Image_Editor(QFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.Anotation_Ref_Gene_Index = -1
    ...
    def Add_Anotation(self):
        ...
        if mt.exec() == QDialog.Accepted:
            self.Anotation_Split_Mode = split_Wid.text()
            self.Anotation_Ref_Gene_Index = Ref.currentIndex()
            x, y, w, h = self.Img_DrawRects[self.Anotation_Ref_Gene_Index]
            Ref_Img = self.Imgs.itemAt(self.Anotation_Ref_Gene_Index).widget().pixmap()
            Ref_Img = Ref_Img.toImage()
            Ref_Img = Ref_Img.copy(x, y, w, h)
            an.SetColumn(self.Anotation_Split_Mode)
    ...
    def display_img(self, Tree_obj):
        ...
        for i in range(Tree_obj.topLevelItemCount()):
            ...
            rect = t.Img_Block_Pre.Pre.rect_fix(t.Img_Block_Pre.Pre.draw_rec[0]['rect'])
    ...
    def Save_Img(self):
        ...
        FileName = QFileDialog.getSaveFileName(self, 'Save as', self.Save_To_Dir, '*.png')[0]
